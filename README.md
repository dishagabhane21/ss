...
<input class="mcq-answer" id="mcq-answer-4" value="26" hidden><p>Quiz: <b>How many (real) swaps are required to sort [29, 10, 14, 37, 13] by Selection Sort?</b></p><form><input type="radio" name="mcq-4-choice" value="26"> 3<br><input type="radio" name="mcq-4-choice" value="24"> 1<br><input type="radio" name="mcq-4-choice" value="25"> 2<br><input type="radio" name="mcq-4-choice" value="27"> 4<br></form><button class="mcq-submit" id="submit-4">Submit</button> <span id="answer-status-4"></span>
<div class='electure-end'>X <u>Esc</u></div>
...
<pre>void insertionSort(int a[], int N) {<br>  for (int i = 1; i &lt; N; i++) { // O(N)<br>    X = a[i]; // X is the item to be inserted<br>    for (j = i-1; j &gt;= 0 &amp;&amp; a[j] &gt; X; j--) // can be fast or slow<br>      a[j+1] = a[j]; // make a place for X<br>    a[j+1] = X; // index j+1 is the insertion point<br>  }<br>}</pre>
<div class='electure-end'>X <u>Esc</u></div>
...
<input class="mcq-answer" id="mcq-answer-5" value="31" hidden><p>Quiz: <b>What is the complexity of Insertion Sort on any input array?</b></p><form><input type="radio" name="mcq-5-choice" value="30"> O(N log N)<br><input type="radio" name="mcq-5-choice" value="28"> O(1)<br><input type="radio" name="mcq-5-choice" value="31"> O(N^2)<br><input type="radio" name="mcq-5-choice" value="29"> O(N)<br></form><button class="mcq-submit" id="submit-5">Submit</button> <span id="answer-status-5"></span><br><br><p>Ask your instructor if you are not clear on this or read similar remarks on <a href="?slide=10-10"><u>this slide</u></a>.</p>
<div class='electure-end'>X <u>Esc</u></div>
...
<p>We will dissect this Quick Sort algorithm by first discussing its most important sub-routine: The O(<b>N</b>) <samp>partition</samp> (classic version).</p><br><p>To partition <b>a[i..j]</b>, we first choose <b>a[i]</b> as the pivot <b>p</b>.<br></p><p>The remaining items (i.e. <b>a[i+1..j]</b>) are divided into 3 regions:</p><ol><li><b>S1</b> = <b>a[i+1..m]</b> where items are &lt; <b>p</b>,</li><li><b>S2</b> = <b>a[m+1..k-1]</b> where items are &le; <b>p</b>, and</li><li>Unknown = <b>a[k..j]</b>, where items are yet to be assigned to either <b>S1</b> or <b>S2</b>.</li></ol><p>Discussion: Why do we choose <b>p</b> = <b>a[i]</b>? Are there other choices?</p><br><p>Harder Discussion: Is it good to always put item(s) that is/are == <b>p</b> on S2 at all times?</p>
<div class='electure-end'>X <u>Esc</u></div>
...
changeSortType(gw.randomizedQuickSort, DEFAULT_DATA);
      ...
      $(function() {
        ...
        $.get('/hasvisited' + '/sorting', function(data) {
          ...
          if (!hasVisited) {
            var postData = {
              '_token': 'ZrcCVNnn8VJ5KvTSulZHNuh7SWYe8ZyTT7Lk2VDK',
